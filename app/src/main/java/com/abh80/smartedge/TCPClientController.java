...
public class TCPClientController {
    ...
    private static final String TAG = "TCPClientController";
    //    Socket
    ...
    //    提供一个全局的静态方法
    public static TCPClientController sharedCenter() {
        if (instance == null) {
            synchronized (TCPClientController.class) {
                if (instance == null) {
                    instance = new TCPClientController();
                }
            }
        }
        return instance;
...
//                    socket.setSoTimeout ( 2 * 1000 );//设置超时时间
                    if (isConnected()) {
                        TCPClientController.sharedCenter().ipAddress = ipAddress;
                        TCPClientController.sharedCenter().port = port;
